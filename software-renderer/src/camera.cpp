...
mat4 getTransform(const camera_data& camera) {
    ...
    transformation *= transform::perspective(camera.fov, 16.0f / 9.0f, 0.1f, 100.f);
    transformation *= transform::view(camera.position, camera.forward, camera.right, camera.up);
    return transformation;
}

vec3 getUp(const camera_data& camera) { return camera.up; }

vec3 getRight(const camera_data& camera) { return camera.right; }

vec3 getForward(const camera_data& camera) { return camera.forward; }

vec3 getPosition(const camera_data& camera) { return camera.position; }

void setPosition(camera_data& camera, const vec3& position) { camera.position = position; }
