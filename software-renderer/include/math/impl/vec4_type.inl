...
template <typename T>
vec<T, 4>::vec(const vec<T, 3>& other, T w) {
    this->data[0] = other.data[0];
    this->data[1] = other.data[1];
...
template <typename T>
vec<T, 4>& vec<T, 4>::operator+=(const vec<T, 4>& v) {
    *this = *this + v;
    return *this;
}
...
template <typename T>
vec<T, 4>& vec<T, 4>::operator*=(float s) {
    *this = *this * s;
    return *this;
}
...
bool operator==(const vec<T, 4>& u, const vec<T, 4>& v) {
    return detail::computeEq<vec<T, 4>, std::is_same<T, float>::value>::call(u, v);
}

template <typename T>
bool operator!=(const vec<T, 4>& u, const vec<T, 4>& v) {
    return detail::computeNeq<vec<T, 4>, std::is_same<T, float>::value>::call(u, v);
}
...
vec<T, 4> operator+(const vec<T, 4>& u, const vec<T, 4>& v) {
    return detail::computeAdd<vec<T, 4>, std::is_same<T, float>::value>::call(u, v);
}

template <typename T>
vec<T, 4> operator-(const vec<T, 4>& u, const vec<T, 4>& v) {
    return detail::computeSub<vec<T, 4>, std::is_same<T, float>::value>::call(u, v);
}

template <typename T>
vec<T, 4> operator*(const float& s, const vec<T, 4>& v) {
    return detail::computeMul<vec<T, 4>, std::is_same<T, float>::value>::call(s, v);
}
...
template <typename T>
vec<T, 4> operator/(const vec<T, 4>& v, const float& s) {
    return detail::computeDiv<vec<T, 4>, std::is_same<T, float>::value>::call(v, s);
}
